...
| DELETE | /playlist/{playlistId}/positions/{position} | Remove a música na **posição** (`position`) informada — índice **0-based** na ordem da tracklist. Apaga **exatamente uma ocorrência** (suporta playlists com duplicatas) e recompacta as posições restantes (0..N-1). Retorna a playlist completa (mesmo shape do `GET`, simetria com add/append/reorder). `404` se a playlist não existir ou a posição estiver fora do intervalo; `400` se `position` não for inteiro. Substitui o antigo `DELETE /playlist/{playlistId}/{musicId}` (que apagava todas as ocorrências). |
